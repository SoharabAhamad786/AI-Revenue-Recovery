"""
RecoverAI - 5-Minute Professional Demonstration Video Production Pipeline
...
Features:
- Full 5-minute timed browser walkthrough of the live web application
- Neural voiceover narration in individual developer perspective ("I built RecoverAI", "Soharab Ahamad")
- Small, clean, professional subtitles positioned strictly at the BOTTOM-LEFT corner (Alignment=1, FontSize=13)
- Deliverables saved directly in project root
"""
...
SCRIPT_FILE = os.path.join(ROOT_DIR, "RecoverAI_Demo_Script.txt")
SRT_FILE = os.path.join(ROOT_DIR, "RecoverAI_Demo_Subtitles.srt")
FINAL_VIDEO_5MIN = os.path.join(ROOT_DIR, "RecoverAI_5Min_Demo.mp4")
FINAL_VIDEO_COMPLETE = os.path.join(ROOT_DIR, "RecoverAI_Complete_Demo.mp4")

TEMP_AUDIO_MP3 = os.path.join(ROOT_DIR, "_temp_rec_audio.mp3")
TEMP_VIDEO_RAW_MP4 = os.path.join(ROOT_DIR, "_temp_rec_screen.mp4")
TEMP_VIDEO_MUX_MP4 = os.path.join(ROOT_DIR, "_temp_rec_mux.mp4")

SECTIONS = [
    {
        "id": 1,
        "title": "Introduction & Author Attribution",
        "text": (
    ...
    {
        "id": 3,
        "title": "Command Center & 3D Financial Galaxy",
        "text": (
...
def log(msg):
    print(f"[RecoverAI Pipeline] {msg}", flush=True)
...
async def generate_narration_and_subtitles():
    ...
    log("Generating small, bottom-left synchronized SRT subtitles...")
    srt_entries = []
    current_time = 0.0
    ...
    with open(SRT_FILE, "w", encoding="utf-8") as f:
        f.write("\n".join(srt_entries))
    log(f"Synchronized subtitles saved to {SRT_FILE}")

    # Also save the clean text script
    with open(SCRIPT_FILE, "w", encoding="utf-8") as f:
        f.write("RecoverAI — Intelligent Invoice and Payment Recovery Assistant\n")
        f.write("5-Minute Technical Demonstration Script\n")
        f.write("Project Owner & Developer: Soharab Ahamad\n")
        f.write("Track: Track 3: AI Revenue Recovery\n\n")
        for s in SECTIONS:
            f.write(f"=== {s['title']} ===\n")
            f.write(s['text'] + "\n\n")
    log(f"Script saved to {SCRIPT_FILE}")
...
def record_walkthrough():
    ...
    raw_video_dir = os.path.join(ROOT_DIR, "_raw_rec_playwright")
...
def composite_final_deliverable():
    log("Compositing video, vocal narration, and burning in small bottom-left subtitles...")
    ...
    log(f"Muxed audio-visual intermediate: {TEMP_VIDEO_MUX_MP4}")

    # 2. Burn in subtitles at BOTTOM-LEFT with small, elegant font
    # Alignment=1 -> Bottom Left
    # MarginL=40 -> 40px from left edge
    # MarginV=25 -> 25px from bottom edge
    # FontSize=13 -> Small, refined, non-intrusive font size
    # BorderStyle=3 -> Soft dark opaque bounding box
    srt_escaped = SRT_FILE.replace("\\", "/").replace(":", "\\:")
    cmd_subtitles = [
        "ffmpeg", "-y",
        "-i", TEMP_VIDEO_MUX_MP4,
        "-vf", f"subtitles='{srt_escaped}':force_style='Alignment=1,MarginL=40,MarginV=25,FontSize=13,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,BackColour=&HA0000000,BorderStyle=3'",
        "-c:a", "copy",
        FINAL_VIDEO_5MIN
    ]
    try:
        subprocess.run(cmd_subtitles, check=True)
        log(f"Generated: {FINAL_VIDEO_5MIN}")
        shutil.copy(FINAL_VIDEO_5MIN, FINAL_VIDEO_COMPLETE)
        log(f"Synchronized duplicate: {FINAL_VIDEO_COMPLETE}")
    except Exception as e:
        log(f"Subtitle burn-in fallback: {e}")
        shutil.copy(TEMP_VIDEO_MUX_MP4, FINAL_VIDEO_5MIN)
        shutil.copy(TEMP_VIDEO_MUX_MP4, FINAL_VIDEO_COMPLETE)

    # Clean up temporary intermediates
    for tmp in [TEMP_AUDIO_MP3, TEMP_VIDEO_RAW_MP4, TEMP_VIDEO_MUX_MP4]:
    ...
    print(f"  * Final Video:     {FINAL_VIDEO_5MIN} (Bottom-Left Small Subtitles)", flush=True)
